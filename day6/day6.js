...
function parseInput(input) {
    ...
    for (let i = 0; i < 4; i++) {
        const lineOfNumbers = input[i].split(" ").filter(x => x).map(n => Number(n));
    ...
    for (let i = 0; i < arrayOfNumberArrays[0].length; i++) {
        let tempArr = [];
        tempArr.push(arrayOfNumberArrays[0][i])
        tempArr.push(arrayOfNumberArrays[1][i])
        tempArr.push(arrayOfNumberArrays[2][i])
        tempArr.push(arrayOfNumberArrays[3][i])
        tempArr.push(arrayOfNumberArrays[4][i])
